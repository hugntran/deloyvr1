...
const ChangeTimeTab: React.FC<ChangeTimeTabProps> = ({ tickets, locationId, isLoading, onReassignSuccess }) => {
  ...
  const handleConfirm = async (ticket: any) => {
    ...
    const url =
      `/app-data-service/slots/valid-by-type?` +
  ...
  const handleAccept = async (ticketId: string) => {
    const token = localStorage.getItem("authToken");
    try {
      const res = await fetch(`/app-data-service/tickets/confirm-change-time/${ticketId}`, {
        method: "PUT",
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
        },
  ...
  const handleReject = async (ticketId: string) => {
    const token = localStorage.getItem("authToken");
    try {
      const res = await fetch(`/app-data-service/tickets/reject-change-time/${ticketId}`, {
        method: "PUT",
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
        },
