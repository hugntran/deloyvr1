...
const SlotListPage = () => {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      const response = await axios.get("/app-data-service/locations/nearby", {
        params: {
  ...
  const fetchSlots = async (locationId: string) => {
    ...
    try {
      const token = getToken();
      const response = await axios.get("/app-data-service/slots/find-pageable-slots", {
  ...
  const updateSlotStatus = async (slot: any) => {
    ...
    try {
      const token = getToken();
      await axios.put(
        `/app-data-service/slots/status/${slot.id}`,
        {
          status: newStatus,
